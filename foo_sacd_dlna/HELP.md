...
## DSD Processor option

### Process DLNA audio through DSD Processor

**Tooltip:** Runs the installed `foo_dsd_processor` as a private DSP chain used only for DLNA preparation. The normal foobar2000 playback DSP chain is not changed.
...
**PCM library to DSD:** configure the DSD Processor for PCM→DSD, e.g. DSD128 or DSD256. The DLNA server will then expose the converted DSF to the renderer.

**Reduce DSD256 network load:** configure DSD Processor to resample DSD256 → DSD128. The network remains DSD, but the sustained audio payload is approximately halved.

The component rejects a DSP configuration that produces ordinary PCM because this project is designed to keep network playback DSD-only.

### Configure DSD Processor

The button opens the installed DSD Processor's normal configuration dialog and stores a **private DLNA preset**. It does not change the user's normal foobar2000 DSP playback chain.

The preset fingerprint is part of the DSF cache key. Therefore:

```text
Change DSP settings
      ↓
New preset fingerprint
      ↓
Old converted DSF cache ignored
      ↓
New DSF generated
```

This prevents a stale DSD cache from being reused after a sample-rate, filter or conversion setting has changed.
