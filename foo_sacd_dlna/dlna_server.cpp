...
void networkLog(const std::string& message) {
    if (!static_cast<bool>(sacd_dlna_cfg::network_logging)) return;
    std::lock_guard<std::mutex> g(g_logMutex);
    FB2K_console_formatter() << "SACD DLNA [network]: " << message;
    try {
...
void setLastError(const std::string& message) {
    if (!static_cast<bool>(sacd_dlna_cfg::network_logging)) return;
    std::lock_guard<std::mutex> g(g_logMutex);
    FB2K_console_formatter() << "SACD DLNA [error]: " << message;
    try {
        const auto path = networkLogPath();
        std::error_code ec;
        if (fs::exists(path, ec) && fs::file_size(path, ec) > 10ull * 1024ull * 1024ull) {
...
std::string SacdDlnaServer::chooseRendererProtocolInfo(const std::string& defaultMime) const {
    ...
    // Prefer the negotiated renderer MIME exactly; otherwise advertise the
    // standard DLNA streaming profile used by this server.
...
std::string SacdDlnaServer::makeDeviceXml() const {
    ...
    return "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
        ...
        "<modelDescription>Native DSD UPnP Media Server</modelDescription><modelNumber>0.7-alpha3</modelNumber>"
...
std::string SacdDlnaServer::browseDidl(const std::string& objectId, bool metadataOnly,
    ...
    if (metadataOnly) {
        ...
        const unsigned end = requestedCount ? std::min<unsigned>(totalMatches, startingIndex + requestedCount) : totalMatches;
...
bool SacdDlnaServer::ensureProcessedDsf(const Item& item, std::wstring& cachePath, DsdTrack& track, abort_callback& abort) {
    ...
    const auto path = cacheFolder() + L"\\dsp-" + pfc::stringcvt::string_wide_from_utf8((key + ".dsf").c_str());
    ...
    try {
        ...
        file_info info; infoReader->get_info(item.subsong, info, abort);
        ...
        if (!_stricmp(item.sourceExt.c_str(), ".iso")) {
            ...
            if (!sacdInput) throw std::runtime_error("foo_input_sacd is not installed or does not handle this SACD ISO");
        ...
        for (;;) {
            ...
            std::vector<audio_chunk> outputs;
            ...
            for (const auto& out : outputs) {
                ...
                if (duration > 0) {
                    ...
                    setConversionStatus(true, static_cast<uint32_t>(std::clamp(seconds / duration * 100.0, 0.0, 100.0)));
                }
            }
        }
        std::vector<audio_chunk> tail;
        ...
        setConversionStatus(false, 100); success = true;
        ...
        setConversionStatus(false, 0); DeleteFileW((path + L".partial").c_str()); DeleteFileW((manifest + L".partial").c_str());
...
bool SacdDlnaServer::ensureCached(const Item& item, std::wstring& cachePath, DsdTrack& track, abort_callback& abort) {
    if (_stricmp(item.sourceExt.c_str(), ".iso") != 0) return false;
    const auto key = cacheKeyFor(item.sourcePath, item.subsong);
    const auto path = cacheFolder() + L"\\" + pfc::stringcvt::string_wide_from_utf8((key + ".dsf").c_str());
    const auto manifest = cacheMetaPath(path);
    uint32_t rate = 0, channels = 0; uint64_t samples = 0, fileSize = 0;
...
bool SacdDlnaServer::serveMedia(SOCKET s, uint32_t itemId, const std::string& requestLine,
    ...
    updateStreamStart(peerIp, track.title, track.dsdRate);
    {
        std::lock_guard<std::mutex> g(m_rateMutex);
        if (sacd_dlna_cfg::stability_mode && track.dsdRate) {
            m_prebufferTargetBytes = std::min<uint64_t>(length, static_cast<uint64_t>(track.dsdRate) / 4ULL * std::clamp<uint32_t>(sacd_dlna_cfg::prebuffer_seconds.get(), 5, 60));
            m_prebufferBytes = std::min<uint64_t>(m_prebufferTargetBytes, 256ull * 1024ull * 1024ull);
        } else {
...
bool SacdDlnaServer::serveAlbumArt(SOCKET s, uint32_t albumId, const std::string& requestLine, abort_callback_impl& aborter) {
    ...
    {
        std::error_code ec; const auto ws = pfc::stringcvt::string_wide_from_utf8(handle->get_path());
        const auto sz = fs::file_size(ws, ec);
        sourceSize = ec ? 0 : static_cast<uint64_t>(sz);
        std::error_code ec2; const auto wt = fs::last_write_time(ws, ec2); sourceTime = ec2 ? 0 : static_cast<int64_t>(wt.time_since_epoch().count());
    }
    ...
    const auto stem = folder + L"\\art-" + pfc::stringcvt::string_wide_from_utf8(keyHex);

    try {
        abort_callback_dummy abort;
        const auto tryDisk = [&](const char* ext, const char* mime) -> bool {
            const std::wstring path = stem + pfc::stringcvt::string_wide_from_utf8(ext);
        ...
        const auto diskPath = stem + pfc::stringcvt::string_wide_from_utf8(ext.c_str());
        {
...
void SacdDlnaServer::httpLoop() {
    ...
    if (bind(m_httpListen, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR || listen(m_httpListen, 16) == SOCKET_ERROR) {
        closesocket(m_httpListen); m_httpListen = INVALID_SOCKET; m_running = false; console::print("SACD DLNA: HTTP bind/listen failed"); return;
    }
    while (m_running) {
...
void SacdDlnaServer::ssdpLoop() {
    ...
    ip_mreq mreq{}; mreq.imr_multiaddr.s_addr = inet_addr("239.255.255.250"); mreq.imr_interface.s_addr = htonl(INADDR_ANY);
    setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
    sockaddr_in multicast{}; multicast.sin_family = AF_INET; multicast.sin_addr.s_addr = inet_addr("239.255.255.250"); multicast.sin_port = htons(1900);
    ...
    auto notifyAlive = [&]() {
        ...
        for (int i = 0; i < 3; ++i) {
            const std::string msg = "NOTIFY * HTTP/1.1\r\nHOST: 239.255.255.250:1900\r\nCACHE-CONTROL: max-age=1800\r\nLOCATION: " + location +
                "\r\nNT: " + nts[i] + "\r\nNTS: ssdp:alive\r\nSERVER: Windows/10 UPnP/1.1 foo_sacd_dlna/0.7-alpha3\r\nUSN: " + usns[i] + "\r\n\r\n";
            sendto(s, msg.data(), static_cast<int>(msg.size()), 0, reinterpret_cast<sockaddr*>(&multicast), sizeof(multicast));
        }
    };
    auto notifyByeBye = [&]() {
        const char* nts[] = { "upnp:rootdevice", kUuid, "urn:schemas-upnp-org:device:MediaServer:1" };
    ...
    while (m_running) {
        if (std::chrono::steady_clock::now() >= nextAlive) { notifyAlive(); nextAlive = std::chrono::steady_clock::now() + std::chrono::seconds(30); }
        if (std::chrono::steady_clock::now() >= nextDiscover) { discover(); nextDiscover = std::chrono::steady_clock::now() + std::chrono::seconds(20); }
        fd_set set; FD_ZERO(&set); FD_SET(s, &set); timeval tv{1,0}; if (select(0, &set, nullptr, nullptr, &tv) <= 0) continue;
        char buf[4096]{}; sockaddr_in from{}; int fromLen = sizeof(from); const int n = recvfrom(s, buf, sizeof(buf) - 1, 0, reinterpret_cast<sockaddr*>(&from), &fromLen); if (n <= 0) continue;
        buf[n] = 0; const std::string msg(buf);
        if (msg.rfind("HTTP/1.1 200", 0) == 0) {
            char peerIp[INET_ADDRSTRLEN]{}; inet_ntop(AF_INET, &from.sin_addr, peerIp, sizeof(peerIp)); discoverSdxResponse(msg, peerIp); continue;
        }
...
void SacdDlnaServer::publish(const metadb_handle_list& items) {
    ...
    for (size_t i = 0; i < items.get_count(); ++i) {
        ...
        {
            std::error_code ec; const auto ws = pfc::stringcvt::string_wide_from_utf8(path);
            const auto sz = fs::file_size(ws, ec);
            x.sourceSize = ec ? 0 : static_cast<uint64_t>(sz);
            std::error_code ec2; const auto wt = fs::last_write_time(ws, ec2);
            x.sourceWriteTime = ec2 ? 0 : static_cast<int64_t>(wt.time_since_epoch().count());
        }
        try {
            abort_callback_dummy abort; service_ptr_t<input_info_reader> infoReader; input_entry::g_open_for_info_read(infoReader, nullptr, path, abort); file_info info; infoReader->get_info(x.subsong, info, abort);
            ...
            x.track.duration = info.get_length(); x.track.dsdRate = static_cast<uint32_t>(std::max<t_int64>(0, info.info_get_int("samplerate")));
            x.track.channels = static_cast<uint32_t>(std::max<t_int64>(1, info.info_get_int("channels")));
            x.track.bitsPerSample = static_cast<uint32_t>(std::max<t_int64>(1, info.info_get_int("bitspersample")));
            ...
            if (x.sourceExt == ".dsf" || x.sourceExt == ".dff") {
                std::error_code ec; const auto ws = pfc::stringcvt::string_wide_from_utf8(x.sourcePath.c_str()); x.track.fileSize = fs::file_size(ws, ec);
            }
            x.track.path = x.sourceExt == ".iso" ? std::wstring{} : pfc::stringcvt::string_wide_from_utf8(x.sourcePath.c_str());
    ...
    std::sort(newAlbums.begin(), newAlbums.end(), [&](const Album& a, const Album& b) {
        ...
        const std::string an = aa ? aa->name : "", bn = bb ? bb->name : "";
    ...
    for (auto& album : newAlbums) std::sort(album.itemIds.begin(), album.itemIds.end(), [&](uint32_t a, uint32_t b) {
        ...
        return (!ia || !ib) ? a < b : ciLess(ia->track.title, ib->track.title);
    });
    for (auto& artist : newArtists) std::sort(artist.albumIds.begin(), artist.albumIds.end(), [&](uint32_t a, uint32_t b) {
        ...
        return (!aa || !bb) ? a < b : ciLess(aa->title, bb->title);
    });
